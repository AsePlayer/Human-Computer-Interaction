...
public class FireExtinguisherNavigation : MonoBehaviour
{
    public GameObject player, pointer, uihelper;
    ...
    void TaskOnClick_Return()
    {
        ...
        uihelper.SetActive(false);
        activated = false;
        GetComponent<GameGenerator>().paused = false;
    }
    ...
    void Update()
    {
        if (OVRInput.Get(OVRInput.Button.Start, OVRInput.Controller.LTouch) && activated == false)
        {
            ...
            activated = true;
            GetComponent<GameGenerator>().paused = true;
        }
